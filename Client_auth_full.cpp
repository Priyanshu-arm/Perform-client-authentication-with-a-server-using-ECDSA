...
int main() {
    try {
        ...
        const char* serial = "CLIENT123456";
        uint8_t hash[32];
        sha256_Raw(reinterpret_cast<const uint8_t*>(serial), strlen(serial),hash);

        uint8_t signature[64];
        ecdsa_sign(&secp256k1, private_key, hash , signature, NULL);
   
        AuthRequest auth = AuthRequest_init_zero;
        strcpy(auth.serial_id, serial);
        memcpy(auth.signature.bytes, signature, 64);
        auth.signature.size = 64;

         uint8_t auth_buf[128];
        pb_ostream_t stream = pb_ostream_from_buffer(auth_buf, sizeof(auth_buf));
        pb_encode(&stream, AuthRequest_fields, &auth);
        boost::asio::write(socket, boost::asio::buffer(auth_buf, stream.bytes_written));
        ...
        std::cout << "[Client] Full auth flow completed."<<std::endl;
